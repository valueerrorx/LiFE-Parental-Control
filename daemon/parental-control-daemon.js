...
function handleClientCommand(client, cmd) {
    ...
    if (cmd.type === 'setup-dnsmasq') {
        ...
        try {
            ...
            const followNmResolvFile = (() => {
                if (!fs.existsSync('/run/NetworkManager/resolv.conf')) return false;
                try {
                    const ip = pickNameserverFromResolv(fs.readFileSync('/run/NetworkManager/resolv.conf', 'utf8'));
                    return !!ip && !isStubNameserver(ip);
                } catch { return false; }
            })();
            ...
            const upstreamLines = followNmResolvFile
                ? ['resolv-file=/run/NetworkManager/resolv.conf']
                : [`server=${nmNameserver}`, 'no-resolv'];
