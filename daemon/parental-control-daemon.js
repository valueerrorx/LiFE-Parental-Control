    ...
    if (cmd.type === 'write-dnsmasq') {
        ...
        try {
            ...
            if (dnsMode === 'dhcp') {
                ...
                upstreamDns = DNS4EU[dnsMode] || DNS4EU['dns4eu_protective'];
            }

            // In dhcp-mode the primary IS the local DNS — no fallback needed.
            // For dns4eu modes always add a fallback so the device stays online if the
            // primary is unreachable (e.g. port 53 blocked on a school network).
            const fallback = dnsMode !== 'dhcp' ? (dhcpFallbackDns || '1.1.1.1') : null;
            ...
            const dnsmasqConf = [
                ...
                'bind-interfaces',
                `server=${upstreamDns}`,
                ...(fallback ? [`server=${fallback}`, 'strict-order'] : []),
                'no-resolv',
                'no-poll',
                'cache-size=1000',
                'domain-needed',
