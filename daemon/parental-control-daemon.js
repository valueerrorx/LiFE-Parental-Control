...
function handleClientCommand(client, cmd) {
    ...
    if (cmd.type === 'write-dnsmasq') {
        ...
        // dns4eu: 86.54.11.1 = protective, 86.54.11.100 = unprotected
        const UPSTREAM_DNS_PROTECTIVE   = '86.54.11.1';
        const UPSTREAM_DNS_UNPROTECTED  = '86.54.11.100';
        try {
            const entries  = Array.isArray(cmd.entries) ? cmd.entries : [];
            const useProtective = cmd.useProtectiveDns !== false; // default: protective
            const upstreamDns = useProtective ? UPSTREAM_DNS_PROTECTIVE : UPSTREAM_DNS_UNPROTECTED;
            ...
            const dnsmasqConf = [
                ...
                '# Forward all DNS queries to dns4eu',
                `server=${upstreamDns}`,
                'no-resolv',
                'no-poll',
                'cache-size=1000',
                '',
                '# Blocked domains (sinkholed to 0.0.0.0)',
                blockedLines,
            ].join('\n') + '\n';
    ...
    if (cmd.type === 'remove-dnsmasq') {
        ...
        try {
            ...
            // Reset dnsmasq.conf to empty/pass-through
    ...
    if (cmd.type === 'setup-dnsmasq') {
        ...
        try {
            ...
            // 2. Remove immutable bit if set, write minimal dnsmasq.conf
            const dnsmasqConf = [
