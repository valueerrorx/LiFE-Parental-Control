...
- **Polkit JS rule**: `50-org.tuxfamily.life-parental-control.rules` matches **`pkexec` command_line** if it contains **`life-parental-control`** OR **`life parental control`** (case-insensitive) so default **AppImage** product-name paths work; README Polkit + Troubleshooting rows updated.
...
- **Backup v1**: bundle includes **`processWhitelist`**; import applies it via `replaceProcessWhitelistFromBackup` (writes `default.json` `quotaExemptions`, quota redeploy, legacy kill cleanup). Example JSON + Settings import hint updated.
- **Quota exemptions** (`default.json` `quotaExemptions`): only skips **daily app quota** kill when limit reached; legacy `life-parental-kill` cron removed on save/redeploy. **Packaged main**: `pkexec /usr/bin/env …` forwards DISPLAY/Wayland/XDG/DBus/APPIMAGE for root GUI relaunch.
...
- **`settings:getConfig`**: returns only whitelisted prefs from `default.json` `preferences`; keeps security fields and stray keys out of renderer.
- **`settings:saveConfig`**: ignores unknown keys — updates only normalized whitelisted prefs in `default.json` `preferences`.
...
- **`repairInvalidLockIdleInConfig`**: on app start, drops invalid `lockIdleMinutes` from `default.json` `preferences`. **`settings:getConfig`** sanitizes IPC payload.
...
- **Backup import**: `Object.hasOwn` on `schedules`, `webFilter`, `blockedApps`, `quotas`, `processWhitelist`, `preferences` — absent key → unchanged; `processWhitelist` non-object → skip; `preferences` non-object → `clearSessionLockPreference` (drop `lockIdleMinutes`); other rules unchanged.
...
- **Navigation**: `MainLayout` refresh on mount; App Control badges (blocked / quotas); Screen Time **on** when `schedule.enabled`. Screen Time **Apply Changes** calls `refreshProtectionsState` so sidebar badges update immediately.
...
- **Auto-lock**: `default.json` `preferences.lockIdleMinutes` (0 / 5 / 15 / 30 / 60), Settings **Session lock**; idle timer on unlock + `life-parental-lock-prefs` event to refresh without re-login. `App.vue` fixes first-run `passwordSet` after `setPassword`.
...
- **Startup merge from `default.json`**: main process calls `applyDefaultMergedState` (`lifeModeIpc.js`) after deferred heavy work — syncs schedule, webfilter mirror, blocked apps, quota entries/exemptions to live enforcement (no separate `life-modes.json`; Schedules page presets only fill the form until **Apply Changes**).
...
- **Schedules page**: School week / Leisure form presets (`applyPreset`); **Apply Changes** persists to `default.json` / enforcement (same as before).
